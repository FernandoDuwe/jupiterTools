...
begin
  ...
  try
    ...
    if vrWizard.Exists('DATABASE_DICTIONARY', ' TABLENAME = ''' + prTableName + ''' AND FIELDNAME = ''' + prField.FieldName + ''' AND HEIGHT IS NOT NULL ') then
    begin
      TDBMemo(Result.Component).Height := StrToInt(vrWizard.Resolve('DATABASE_DICTIONARY', 'HEIGHT', ' TABLENAME = ''' + prTableName + ''' AND FIELDNAME = ''' + prField.FieldName + ''' '));
      Result.Bottom := TDBMemo(Result.Component).Top + TDBMemo(Result.Component).Height;
    end;
  finally
    FreeAndNil(vrWizard);
  end;
